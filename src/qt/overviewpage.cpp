...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeLatinexAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLatinexAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeLatinexAmount = strAnonymizeLatinexAmount.remove(strAnonymizeLatinexAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLatinexAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeLatinexAmount * COIN) nMaxToAnonymize = nAnonymizeLatinexAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeLatinexAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeLatinexAmount));
        strAnonymizeLatinexAmount = strAnonymizeLatinexAmount.remove(strAnonymizeLatinexAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLatinexAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
